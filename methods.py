...
class DeepQNetwork:
    ...
    def __init__(
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            self.q_values = full_module(
                self.input_states, convs, fully_connected,
            ...
            self.loss = tf.losses.huber_loss(
                self.q_targets, self.q_values_selected, delta=gradient_clip)
...
class DuelingDeepQNetwork:
    ...
    def __init__(
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            self.v_values = full_module(
                val, [], fully_connected, 1, activation_fn)
            self.a_values = full_module(
                adv, [], fully_connected, num_actions, activation_fn)

            a_values_mean = tf.reduce_mean(
                self.a_values, axis=1, keepdims=True)
...
class CategoricalDeepQNetwork:
    ...
    def __init__(
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            self.z = np.linspace(
                start=self.v_min, stop=self.v_max, num=num_atoms)
            ...
            self.q_values = tf.reduce_sum(
                tf.multiply(self.probs, self.z), axis=2)
            ...
            # define loss function and update rule
            self.probs_targets = tf.placeholder(
                dtype=tf.float32, shape=[None, self.num_atoms])
            self.loss = -tf.reduce_sum(
                self.probs_targets * tf.log(self.probs_selected+1e-6))
            self.update_model = optimizer.minimize(self.loss)
...
###############################################################################
########################### Quantile Deep Q-Network ###########################
###############################################################################


class QuantileDeepQNetwork:
    ...
    def __init__(
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            tau_max = 1 - tau_min
            tau_vector = tf.lin_space(
                start=tau_min, stop=tau_max, num=num_atoms)

            # reshape tau to matrix for fast loss calculation
            tau_matrix = tf.tile(tau_vector, [num_atoms])
            self.tau_matrix = tf.reshape(
                tau_matrix, shape=[num_atoms, num_atoms])
            ...
            self.atoms_matrix = tf.reshape(
                atoms_matrix, shape=[-1, num_atoms, num_atoms])

            # reshape target atoms to matrix for fast loss calculation
            self.atoms_targets = tf.placeholder(
                dtype=tf.float32, shape=[None, num_atoms])
            targets_matrix = tf.tile(self.atoms_targets, [1, num_atoms])
            targets_matrix = tf.reshape(
                targets_matrix, shape=[-1, num_atoms, num_atoms])
            ...
            delta_atoms_diff = tf.where(
                ...
                tf.ones_like(atoms_diff))
            huber_weights = tf.abs(
                self.tau_matrix - delta_atoms_diff) / num_atoms
            self.loss = tf.losses.huber_loss(
                self.targets_matrix, self.atoms_matrix, weights=huber_weights,
...
class SoftActorCriticNetwork:
    ...
    def __init__(self, num_actions, state_shape=[8, 8, 5],
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            v_loss = tf.losses.huber_loss(
                self.v_targets[:, None], self.v_values)
