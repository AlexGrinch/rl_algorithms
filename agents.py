...
class Agent:
    ...
    def init_weights(self):
        ...
        global_vars = tf.global_variables(
            scope="agent") + tf.global_variables(scope="target")
    ...
    def train(self,
        ...
        with tf.Session(config=config) as sess:
            ...
            if from_epoch == 0:
                ...
                train_rewards = list(
                    np.load(self.path+"/learning_curve.npz")["r"])
                frame_counts = list(
                    np.load(self.path+"/learning_curve.npz")["f"])
            ...
            while num_epochs < max_num_epochs:
                ...
                for time_step in range(self.max_ep_length):
                    ...
                    self.rep_buffer.store_effect(
                        last_idx, action, reward, done)
                    ...
                    # save network wieghts checkpoint and learning curve
                ...
                if episode_count % performance_print_freq == 0:
                    avg_reward = np.mean(
                        train_rewards[-performance_print_freq:])
                    avg_lifetime = np.mean(
                        ep_lifetimes[-performance_print_freq:])
    ...
    def update_agent_weights(self, sess, batch):
        ...
        q_double = self.target_net.get_q_values_sa(
            sess, batch.s_, agent_actions)
...
class DQNAgent(Agent):
    ...
    def __init__(
            self, env, num_actions, state_shape=[8, 8, 5],
        ...
        super(DQNAgent, self).__init__(
            env, num_actions, state_shape=state_shape,
            save_path=save_path, model_name=model_name)

        tf.reset_default_graph()
        self.agent_net = DeepQNetwork(
            self.num_actions, state_shape=state_shape,
...
class DuelDQNAgent(Agent):
    ...
    def __init__(
        ...
        super(DuelDQNAgent, self).__init__(
            env, num_actions, state_shape=state_shape,
            save_path=save_path, model_name=model_name)

        tf.reset_default_graph()
        self.agent_net = DuelingDeepQNetwork(
            self.num_actions, state_shape=state_shape,
            convs=convs, fully_connected=fully_connected,
            activation_fn=activation_fn, optimizer=optimizer,
            gradient_clip=gradient_clip, scope="agent")
...
class CatDQNAgent(Agent):
    ...
    def __init__(
            self, env, num_actions, state_shape=[8, 8, 5],
            convs=[[16, 2, 1], [32, 1, 1]], fully_connected=[128],
            activation_fn=tf.nn.relu, num_atoms=21, v=(-10, 10),
            optimizer=tf.train.AdamOptimizer(2.5e-4, epsilon=0.01/32),
            save_path="rl_models", model_name="CatDQN"):

        super(CatDQNAgent, self).__init__(
            env, num_actions, state_shape=state_shape,
            save_path=save_path, model_name=model_name)
    ...
    def update_agent_weights(self, sess, batch):
        ...
        agent_actions = self.agent_net.get_q_argmax(sess, batch.s_)
        probs_targets = self.target_net.cat_proj(
            sess, batch.s_, agent_actions, batch.r,
...
class QuantRegDQNAgent(Agent):
    ...
    def __init__(
        ...
        super(QuantRegDQNAgent, self).__init__(
            env, num_actions, state_shape=state_shape,
            save_path=save_path, model_name=model_name)

        tf.reset_default_graph()
        self.agent_net = QuantileDeepQNetwork(
            self.num_actions, state_shape=state_shape,
            convs=convs, fully_connected=fully_connected,
            activation_fn=tf.nn.relu, num_atoms=num_atoms,
            kappa=kappa, optimizer=optimizer, scope="agent")
        self.target_net = QuantileDeepQNetwork(
            self.num_actions, state_shape=state_shape,
            convs=convs, fully_connected=fully_connected,
            activation_fn=tf.nn.relu, num_atoms=num_atoms,
    ...
    def update_agent_weights(self, sess, batch):
        ...
        agent_actions = self.agent_net.get_q_argmax(sess, batch.s_)
        next_atoms = self.target_net.get_atoms_sa(
            sess, batch.s_, agent_actions)
        target_atoms = batch.r[:, None] + \
            self.gamma * next_atoms * (1 - batch.done[:, None])
        # update agent network
...
class SACAgent(Agent):
    ...
    def __init__(
            ...
            activation_fn=tf.nn.relu,
            temperature=1.0,
            optimizers=[tf.train.AdamOptimizer(2.5e-4, epsilon=0.01/32),
                        tf.train.AdamOptimizer(2.5e-4, epsilon=0.01/32),
                        tf.train.AdamOptimizer(2.5e-4, epsilon=0.01/32)],
        ...
        tf.reset_default_graph()
        self.agent_net = SoftActorCriticNetwork(
            self.num_actions, state_shape=state_shape,
            convs=convs, fully_connected=fully_connected,
            activation_fn=activation_fn,
            optimizers=optimizers, scope="agent")
        self.target_net = SoftActorCriticNetwork(
            self.num_actions, state_shape=state_shape,
            convs=convs, fully_connected=fully_connected,
            activation_fn=activation_fn,
    ...
    def update_agent_weights(self, sess, batch):
        ...
        v_values_next = self.target_net.get_v_values_s(
            sess, batch.s_).reshape(-1)
        ...
        q_targets = batch.r / self.t + \
            self.gamma * v_values_next * (1 - batch.done)
